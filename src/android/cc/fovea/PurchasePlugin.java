 ...
 *
 * <p>The plugin has methods for:
 *
 * <p>- initializing the Google Play BillingClient,
...
public final class PurchasePlugin
   ...
   * This allows us to use {@code List<ProductDetails>} internally while adapting to
  ...
  /** Convert a single purchase to JSON. */
  private JSONObject toJson(final Purchase p) throws JSONException {
    return new JSONObject(p.getOriginalJson())
        ...
        .put("accountId",
            p.getAccountIdentifiers().getObfuscatedAccountId())
        .put("profileId",
            p.getAccountIdentifiers().getObfuscatedProfileId())
      ...
      mBillingClient.queryPurchasesAsync(
          ...
          new PurchasesResponseListener() {
            public void onQueryPurchasesResponse(
              ...
              Log.i(mTag, "queryPurchases(INAPP) -> Elapsed time: "
                  + (System.currentTimeMillis() - time) + "ms");
              if (billingResult.getResponseCode() == BillingResponseCode.OK) {
                allPurchases.addAll(purchases);
              }
              if (mInAppResult != null
                  && (mSubsResult != null || !areSubscriptionsSupported())) {
                BillingResult finalResult =
                    mInAppResult.getResponseCode() == BillingResponseCode.OK
                        ? mInAppResult : mSubsResult;
                onQueryPurchasesFinished(finalResult, allPurchases);
              }
            }
          }
      );
        ...
        mBillingClient.queryPurchasesAsync(
            ...
            new PurchasesResponseListener() {
              public void onQueryPurchasesResponse(
                ...
                Log.i(mTag, "queryPurchases(SUBS) -> Elapsed time: "
                    + (System.currentTimeMillis() - time) + "ms");
                if (billingResult.getResponseCode() == BillingResponseCode.OK) {
                  allPurchases.addAll(purchases);
                }
  ...
  private JSONObject productDetailsToJson(
      ProductDetails product) throws JSONException {
    ...
    if (product.getProductType().equals(ProductType.INAPP)) {
      ...
      if (offerList != null && offerList.size() > 1) {
        ...
        // Single offer - use legacy v11.0 format
      ...
      for (SubscriptionOfferDetails details :
        ...
        JSONObject offer = new JSONObject()
            ...
            .put("tags",
                new JSONArray(details.getOfferTags()));
  ...
  private void getAvailableProducts(
    ...
    queryAllProductDetails(inAppProductIds, subsProductIds,
        new InternalProductDetailsResponseListener() {
          ...
          public void onProductDetailsResponse(
            ...
            if (result.getResponseCode() != BillingResponseCode.OK) {
              Log.d(mTag, "getAvailableProducts() -> Failed: "
                  + format(result));
              callError(callbackContext, Constants.ERR_LOAD,
                  "Failed to load Products, code: "
                      + result.getResponseCode());
              return;
            }
            ...
            try {
              ...
              Log.d(mTag,
                  "getAvailableProducts() -> Failed: "
                      + e.getMessage());
              callError(callbackContext,
                  Constants.ERR_LOAD, e.getMessage());
            }
          }
        });
  }
  ...
  private BillingFlowParams parseBillingFlowParams(JSONArray data) throws JSONException {
    ...
    // the user's account in your app. Max 64 characters.
    //
    // We pass the UUID generated by Store.userIdToUUID() — a deterministic,
    // reversible hex encoding of the userId. This allows the server-side
    // webhook handler to recover the original userId via uuidToUserId().
    // Google recommends a one-way hash, but we use a reversible encoding
    // to achieve parity with Apple's appAccountToken pattern.
    ...
    // If not passed in additionalData, use the offer token
    // based on the selected offer index.
    ...
    List<ProductDetailsParams> productDetailsParamsList =
        new ArrayList<ProductDetailsParams>();
    ...
    if (offerToken != null) {
      productDetailsParamsList.add(
          ProductDetailsParams.newBuilder()
              ...
              .build());
      Log.d(mTag, "Product details id@token: "
    ...
    final BillingFlowParams.Builder params =
    ...
    if (accountId != null) {
      ...
      // The accountId is the UUID generated by Store.userIdToUUID().
      // It is passed as-is (no MD5 hash) so the server can reverse it
      // back to the original userId via uuidToUserId().
    ...
    if (replacementMode != null) {
      int mode = parseReplacementMode(replacementMode);
      if (mode >= 0) {
        subscriptionUpdateParams
  ...
  /**
   * Parse a replacement mode string into the corresponding int constant.
   *
   * @param mode the replacement mode name
   * @return the ReplacementMode constant, or -1 if unknown
   */
  private int parseReplacementMode(String mode) {
    final int rm;
    switch (mode) {
      case "IMMEDIATE_WITH_TIME_PRORATION":
        rm = BillingFlowParams.SubscriptionUpdateParams
  ...
  public void initiatePurchaseFlow(final BillingFlowParams params) {
    ...
    if (activity == null || activity.isFinishing()) {
      Log.e(mTag, "Activity is null or finishing, "
          + "cannot launch billing flow.");
      callError(Constants.ERR_COMMUNICATION,
          "Activity not available to launch billing flow.");
      return;
    }
    executeServiceRequest(() -> {
      if (getLastResponseCode() != BillingResponseCode.OK) {
      ...
      activity.runOnUiThread(() -> {
        ...
        Log.d(mTag, "launchBillingFlow immediate result: "
            + format(billingResult));
        if (billingResult.getResponseCode()
            != BillingResponseCode.OK) {
          Log.e(mTag,
              "launchBillingFlow failed immediately: "
                  + format(billingResult));
        }
      });
    });
  ...
  private void consumePurchase(
      final String purchaseToken) throws JSONException {
    Log.d(mTag, "consumePurchase(" + purchaseToken + ")");
    if (mTokensToBeConsumed.contains(purchaseToken)) {
      Log.i(mTag,
          "consumePurchase() -> Consume already in progress.");
  ...
  public void onAcknowledgePurchaseResponse(
    ...
    if (result.getResponseCode() == BillingResponseCode.OK) {
      Log.d(mTag,
          "onAcknowledgePurchaseResponse() -> Success");
      callSuccess();
    } else {
      Log.d(mTag,
          "onAcknowledgePurchaseResponse() -> Failed: "
              + format(result));
  ...
  public void onConsumeResponse(
      BillingResult result, String purchaseToken) {
    try {
  ...
  private int productDetailsQuerySuccessCount = 0;

  /**
   * Load in-app products information.
   *
   * @param inAppProductIds list of in-app product IDs
   * @param subsProductIds list of subscription product IDs
  ...
  private void queryAllProductDetails(
        ...
        new InternalProductDetailsResponseListener() {
          ...
          public void onProductDetailsResponse(
            ...
            if (result.getResponseCode()
              ...
              productDetailsQuerySuccessCount++;
              if (productDetailsQuerySuccessCount == nRequests
                  && listener != null) {
                Log.d(mTag,
                    "queryAllProductDetails() -> "
                        + "Calling listener.");
    ...
    if (subsList.size() > 0) {
      Log.d(mTag,
          "queryAllProductDetails() -> Query SUBS.");
      queryProductDetailsAsync(subsList, queryListener);
    }

    if (inAppList.size() > 0) {
      Log.d(mTag,
          "queryAllProductDetails() -> Query INAPP.");
    ...
    if (inAppList.size() == 0 && subsList.size() == 0) {
      Log.d(mTag,
          "queryAllProductDetails() -> "
              + "Calling listener (0 requests).");
      if (listener != null) {
        listener.onProductDetailsResponse(
            getLastResult(), allProducts);
      }
    }
  }
  ...
  public void queryProductDetailsAsync(
    ...
    executeServiceRequest(() -> {
      if (getLastResponseCode() != BillingResponseCode.OK) {
        ...
        Log.d(mTag,
            "queryProductDetailsAsync() -> Success");
  ...
  /** Call success on a specific callback context. */
  private void callSuccess(
      CallbackContext callbackContext, JSONArray array) {
  ...
  /** Call error on a specific callback context. */
   ...
   *
   * <p>Once connected, {@code executeOnSuccess} will be executed.
   *
   * @param executeOnSuccess some code to run once connected
   * @param executeOnFailure some code to run on failure
   */
  public void startServiceConnection(
      final Runnable executeOnSuccess,
    ...
    mBillingClient.startConnection(
        new BillingClientStateListener() {
          ...
          private void onBillingConnectionSuccess() {
            Log.d(mTag,
                "startServiceConnection() -> Success");
  ...
  private void executeServiceRequest(final Runnable runnable) {
    if (mIsServiceConnected) {
      ...
      Log.d(mTag,
          "executeServiceRequest() -> Failed (try again).");
